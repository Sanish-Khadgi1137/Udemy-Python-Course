...
class QuizBrain:
    ...
    def next_question(self):# if this line is intended inside the above methode it also execute while calling the class; and while calling this method get error saying "AttributeError: 'QuizBrain' object has no attribute 'next_question'" 
            ...
            user_answer=input(f"Q.{self.question_number}: {current_question.text} (True/False): ")#"self.question_number" give current question number and "current_question.text" gives question
            self.check_answer(user_answer, current_question.answer)#correct_answer = current.question.answer

    def check_answer(self, user_answer, correct_answer):
        if user_answer.lower() == correct_answer.lower():
            print("You go it right!")
            self.score += 1 #(tracked) if correct answer the self.score gets increased by 1
        else:
            print("That's wrong.")
        print(f"The correct answer is {correct_answer}.")#this is out of the if/else so print this statement for both cases
